...
const Restaurant = ({data}: {RestaurantType}) => {
  ...
  return <>
    ...
    <div>
      <h3>Menu:</h3>
      <ul>{menu.map((menuItem) => <Menu data={menuItem} key={menuItem.id} />)}</ul>
    </div>
    <div>
      <h3>Reviews:</h3>
      {<ul>{reviews.map((review) => <Review data={review} key={review.id} />)}</ul>}
    </div>
  </>
}
...
createRoot(document.getElementById('root')!).render(
  <StrictMode>
    <h1>Restaurants:</h1>
    {restaurants.map((restaurant) => <Restaurant data={restaurant} key={restaurant.id} />)}
  </StrictMode>
)
